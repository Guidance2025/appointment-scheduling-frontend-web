...
const ExitInterview = () => {
  ...
  const handlePost = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/exit-interview/create/${guidanceStaffId}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const handleUpdateQuestion = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/exit-interview/questions/${questionId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
  ...
  const filteredResponses = responsesData
    .filter(item => {
      ...
      return (questionText.includes(searchLower) || responseText.includes(searchLower))
          && filterByDateRange(item.submittedDate, filterDate);
    })
    .sort((a, b) => new Date(b.submittedDate) - new Date(a.submittedDate));
